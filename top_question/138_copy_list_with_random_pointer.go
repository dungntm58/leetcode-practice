...
func CopyRandomList(head *Node) *Node {
	nodeMap, zero := make(map[*Node]*Node), &Node{}
	curr, clone := head, zero
	for curr != nil {
		clone.Next = &Node{curr.Val, nil, nil, nil, nil, nil}
		nodeMap[curr] = clone.Next
		curr, clone = curr.Next, clone.Next
	}
	curr, clone = head, zero.Next
	for curr != nil {
		if curr.Random != nil {
			nodeMap[curr].Random = nodeMap[curr.Random]
		} else {
			nodeMap[curr].Random = nil
		}
		curr, clone = curr.Next, clone.Next
	}
	return zero.Next
