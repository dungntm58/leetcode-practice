...
func RobotSim(commands []int, obstacles [][]int) int {
	...
	for _, ob := range obstacles {
		obstaclesSet[hash(ob[0], ob[1])] = true
	}
	...
	for _, command := range commands {
		...
		default:
			...
			for i := 0; i < command; i++ {
				newX, newY := currentPosX+direction[0], currentPosY+direction[1]
				if _, ok := obstaclesSet[hash(newX, newY)]; ok {
					break
				}
				currentPosX, currentPosY = newX, newY
			}
			res = max(res, currentPosX*currentPosX+currentPosY*currentPosY)
		}
	}
	return res
}
